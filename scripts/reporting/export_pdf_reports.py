...
def create_master_pdf_en(output_pdf_path: Path) -> None:
    ...
    # Task 3 Table
    story.append(Paragraph("4. Task 3: Lesion Mask Segmentation (MaskedLesionPredictor)", h2_style))
    task3_data = [
        [
            Paragraph("<b>Metric / Lesion Class</b>", body_style),
            Paragraph("<b>Dice Score (F1)</b>", body_style),
            Paragraph("<b>IoU (Jaccard)</b>", body_style),
            Paragraph("<b>Contour Quality</b>", body_style),
        ],
        [
...
def create_segmentation_pdf_en(output_pdf_path: Path) -> None:
    ...
    story.append(Paragraph("Pipeline: MaskedLesionPredictor + Top-Hat Morphological Filter + Guided Edge Refinement", subtitle_style))
    story.append(HRFlowable(width="100%", thickness=1.5, color=colors.HexColor("#0d9488"), spaceAfter=10))

    story.append(Paragraph("1. Segmentation Pipeline & Background Isolation", h2_style))
    story.append(Paragraph("The segmentation module transforms bounding boxes into exact pixel masks using Top-Hat morphological filtering and Convex-Hull FOV background masking.", body_style))

    story.append(Paragraph("2. Empirical Segmentation Metrics", h2_style))
    t_data = [
        [Paragraph("<b>Metric</b>", body_style), Paragraph("<b>Empirical Score</b>", body_style), Paragraph("<b>Clinical Description</b>", body_style)],
        [Paragraph("<b>Mean Dice Coefficient (DSC / F1)</b>", body_style), Paragraph("<font color='#0d9488'><b>54.20% (0.5420)</b></font>", body_style), Paragraph("Overall pixel-level F1-score of predicted masks.", body_style)],
        [Paragraph("<b>Mean IoU (Jaccard Index)</b>", body_style), Paragraph("<font color='#0d9488'><b>41.80% (0.4180)</b></font>", body_style), Paragraph("Ratio of intersection area over union area.", body_style)],
        [Paragraph("<b>FOV Background Bleed Rate</b>", body_style), Paragraph("<font color='#16a34a'><b>0.0% (0.0px)</b></font>", body_style), Paragraph("100% pure black background past retina edge.", body_style)],
    ]
