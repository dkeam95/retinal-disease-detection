...
class LesionDetectionDataset(Dataset[tuple[torch.Tensor, dict[str, torch.Tensor]]]):
    ...
    def __init__(
        ...
        Args:
            ...
            target_size: Resize target dimensions (height, width). Default: (640, 640).
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        # Scale bounding box coordinates to match target_size
        ...
        for box, label in zip(annotation.boxes, annotation.labels, strict=False):
            xmin = max(0.0, box[0] * scale_x)
            ymin = max(0.0, box[1] * scale_y)
            xmax = min(float(target_w), box[2] * scale_x)
            ymax = min(float(target_h), box[3] * scale_y)

            if xmax > xmin + 1.0 and ymax > ymin + 1.0:
                scaled_boxes.append([xmin, ymin, xmax, ymax])
                labels.append(label)

        # Convert image to FloatTensor [0, 1] normalized with ImageNet stats
