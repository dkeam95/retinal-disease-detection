...
Grad-CAM++ uses the exponential-score formulation to obtain the higher-order weighting terms from
feature-map gradients. It is a class-attribution map, not evidence of pixel-level lesion boundaries.
"""
...
class GradCAMPlusPlus:
    ...
    def generate(
        ...
        # Exponential-score formulation yields the Grad-CAM++ higher-order weights
        # from a single backward pass without unstable raw higher derivatives.
        score_exp = torch.exp(score - score.detach())
        score_exp.backward()
