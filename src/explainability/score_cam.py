...
noisy or suffer from saturation in deep networks, Score-CAM perturbs the input image with normalized
...
class ScoreCAM:
    ...
    def _black_baseline(self, input_tensor: torch.Tensor) -> torch.Tensor:
        """Return black RGB represented in ImageNet-normalized tensor space."""
        ...
        return (-mean / std).expand_as(input_tensor)
    ...
    def generate(
        ...
        for i in range(0, num_channels, self.batch_size):
            ...
            # Zero in normalized tensor space is the ImageNet mean colour, not
            # black. Interpolate from black so feature perturbations are valid.
            baseline = self._black_baseline(input_tensor)
            masked_inputs = baseline + (input_tensor - baseline) * batch_masks
