...
def clahe_enhance(
    ...
    Outer non-retina background is explicitly masked to pitch black (0, 0, 0).
    """
    ...
    # Zero out outer background pixels so background is pitch black
    from preprocessing.roi import get_retina_binary_mask
    retina_mask = get_retina_binary_mask(image_rgb)
    if retina_mask.shape[:2] != image_rgb.shape[:2]:
        retina_mask = cv2.resize(retina_mask, (image_rgb.shape[1], image_rgb.shape[0]))
    retina_mask_3ch = np.repeat(retina_mask[:, :, None] > 0, 3, axis=2)
    enhanced[~retina_mask_3ch] = 0
...
def draw_spotlight(
    ...
    # Zero out outer background pixels so background around retina is pitch black
    from preprocessing.roi import get_retina_binary_mask
    retina_mask = get_retina_binary_mask(image_rgb)
    if retina_mask.shape[:2] != (h, w):
        retina_mask = cv2.resize(retina_mask, (w, h))
    retina_mask_3ch = np.repeat(retina_mask[:, :, None] > 0, 3, axis=2)
    canvas_bgr[~retina_mask_3ch] = 0
...
def create_4panel(
    ...
    retina_mask = cv2.resize(get_retina_binary_mask(original_rgb), (w, h))
    if retina_mask.dtype != np.uint8:
        retina_mask = (retina_mask > 0).astype(np.uint8) * 255
    else:
        retina_mask = (retina_mask > 0).astype(np.uint8) * 255
    ...
    # Zero out non-retina background and low activation noise (< 0.08) so background is clean
    active_mask = ((retina_mask > 0) & (hm >= 0.08)).astype(np.uint8) * 255
    clean_colored_bgr = cv2.bitwise_and(colored_bgr, colored_bgr, mask=active_mask)
    clean_colored_rgb = cv2.cvtColor(clean_colored_bgr, cv2.COLOR_BGR2RGB)

    blend = cv2.addWeighted(original_rgb, 0.55, clean_colored_rgb, 0.45, 0)
    # Mask out outer black background circle
    retina_mask_3ch = np.repeat(retina_mask[:, :, None] > 0, 3, axis=2)
    blend[~retina_mask_3ch] = 0
