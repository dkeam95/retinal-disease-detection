...
from detection.detector_model import DETECTION_PIPELINE_VERSION, build_lesion_detector
...
class LesionDetectionPredictor:
    ...
    def __init__(
        ...
        suppress_optic_disc: bool = False,
    ) -> None:
        ...
        Args:
            ...
            target_size: Deprecated compatibility argument. Faster R-CNN resizes
                internally while preserving aspect ratio.
            enable_clahe: Apply the same CLAHE preprocessing used for training.
            suppress_optic_disc: Suppress EX/SE detections in the optic disc.
        """
        ...
        self.suppress_optic_disc = suppress_optic_disc
    ...
    def predict(
        ...
        if self.enable_clahe:
            ...
            l_enhanced = cv2.createCLAHE(
                clipLimit=2.0, tileGridSize=(8, 8)
            ).apply(l_chan)
            rgb = cv2.cvtColor(
                cv2.merge((l_enhanced, a_chan, b_chan)), cv2.COLOR_LAB2RGB
            )

        # Faster R-CNN's GeneralizedRCNNTransform performs its own ImageNet
        # normalization and aspect-ratio preserving resize, then restores boxes
        # to these original-image coordinates.
        orig_h, orig_w = rgb.shape[:2]
        img_tensor = torch.from_numpy(rgb).permute(2, 0, 1).float() / 255.0

        with torch.no_grad():
            outputs = self.model([img_tensor.to(self.device)])[0]
        ...
        for box, score, label in zip(boxes, scores, labels, strict=False):
            ...
            xmin = max(0.0, min(float(box[0]), float(orig_w - 1)))
            ymin = max(0.0, min(float(box[1]), float(orig_h - 1)))
            xmax = max(xmin + 1.0, min(float(box[2]), float(orig_w)))
            ymax = max(ymin + 1.0, min(float(box[3]), float(orig_h)))

            res_boxes.append([xmin, ymin, xmax, ymax])
            res_scores.append(float(score))
            res_labels.append(int(label))

        merged_boxes, merged_scores, merged_labels = res_boxes, res_scores, res_labels
        if self.suppress_optic_disc:
