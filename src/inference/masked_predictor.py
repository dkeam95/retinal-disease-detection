"""Bounding-box-derived lesion mask candidates with FOV coordinate re-mapping."""
...
class MaskedDetectionResult:
    """Dataclass storing detections and heuristic pixel-mask candidates.
    ...
    Attributes:
        ...
        masks: List of 2D uint8 candidate masks of shape (H_orig, W_orig) [0, 255].
    ...
    mask_provenance: str = "bbox_ellipse_heuristic"


class MaskedLesionPredictor:
    """Generate FOV-clipped, bbox-derived mask candidates for visual review.

    These masks are not outputs of a trained pixel-segmentation network and must
    not be scored or presented as lesion boundaries without a mask-label evaluation.
    """
    ...
    def predict_masked(
        ...
        for box, label in zip(raw_det.boxes, raw_det.labels, strict=False):
            ...
            # 2. Generate a visual candidate inside the detection box. This is a
            # heuristic ellipse, not a predicted segmentation mask.
            ...
            # 3. Apply visual post-processing to the candidate mask.
